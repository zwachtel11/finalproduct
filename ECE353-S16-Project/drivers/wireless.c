...
{
				uint8_t data1[2];
				uint8_t data2[2] = {0xFF, 0xFF};
				
				uint8_t rd_addr = reg & 0x1F;
				
				data1[0] = rd_addr;
				data1[1] = 0xFF;
				
				wireless_CSN_low();
  spiTx(RF_SPI_BASE, data1, 2, data2);
				wireless_CSN_high();
...
static __INLINE void wireless_set_tx_addr(uint8_t  *tx_addr)
{
  ...
  data1[5] = *(tx_addr + 4);
	...
	wireless_CSN_low();
  spiTx(RF_SPI_BASE, data1, 6, data2);
	wireless_CSN_high();
}
...
static __INLINE void wireless_tx_data_payload( uint32_t data)
{
	uint8_t data1[5];
	uint8_t data2[5];
	data1[0] = NRF24L01_CMD_W_TX_PAYLOAD;
	data1[1] = (data>>24);
	data1[2] = (data>>16);
	data1[3] = (data>>8);
	data1[4] = (data);
	wireless_CSN_low();
  spiTx(RF_SPI_BASE,data1, 5, data2);
  wireless_CSN_high();
...
static __INLINE void wireless_rx_data_payload( uint32_t *data)
{
	...
	uint8_t data1[5];
	uint8_t data2[5];
	*data1 = NRF24L01_CMD_R_RX_PAYLOAD;
	wireless_CSN_low();
  spiTx(RF_SPI_BASE,data1, 5, data2);
  wireless_CSN_high();
	*data = (data2[1]<<24)|(data2[2]<<16)|(data2[3]<<8)|(data2[4]); 
}
